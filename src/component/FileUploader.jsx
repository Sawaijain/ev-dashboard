...
export const FileUploader = ({ onFileProcess, isLoading, error }) => {
    ...
    return (
      <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
        ...
        >
          <input {...getInputProps()} />
          <Upload className={`w-12 h-12 ${isDragActive ? "text-green-500" : "text-green-400"} mb-4`} />
          {isLoading ? (
            <p className="text-green-900 text-lg font-medium">Processing file...</p>
          ) : (
            <>
              <p className="text-green-900 text-lg font-medium text-center">
                {isDragActive ? "Drop your CSV file here!" : "Drop your CSV file here, or click to select"}
              </p>
              <p className="text-green-500 text-sm mt-2">Supports CSV files only</p>
            </>
          )}
        </div>
        {error && (
